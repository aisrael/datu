...
/// head command implementation: print the first N lines of an Avro, CSV, JSON, Parquet, or ORC file.
pub async fn head(args: HeadsOrTails) -> Result<()> {
    ...
    if !input_file_type.supports_pipeline_display_input() {
        bail!("Only {DISPLAY_PIPELINE_INPUTS_FOR_CLI} are supported for head");
    }
